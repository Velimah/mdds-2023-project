...
//currentTime.setHours(14, 30);
//console.log(currentTime);

const loopMovies = (movies) => {
    ...
    filterMovieByTime.forEach(movie => {
        ...
        if(listingCounter < 4){
            ...
            if(endTime.getHours() > currentTime.getHours() || endTime.getHours() === currentTime.getHours() && endTime.getMinutes() >= currentTime.getMinutes()){
                renderMovie(movie, time, endTime);
...
const refreshListRender = () => {
    ...
    //if movieList is empty, render first four past programs
    if(movieList.childElementCount === 0){
        ...
        movies.forEach(movie => {
            ...
            if(listingCounter < 4){
                const time = new Date(movie.time);
                const endTime = new Date(movie.endTime);
                document.getElementById('comingUpNext').innerText = 'In program';
                renderMovie(movie, time, endTime);
